...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim
        self.reduction = nn.Linear(4 * dim, 2 * dim, bias=False) if dim_scale==2 else nn.Linear(4 * dim, dim, bias=False)
...
class AdaIN1d(nn.Module):
    ...
    def forward(self, x, s):
        # 输入 x 的形状是 (B, C, L)
        # 输入 s 的形状是 (B, style_dim)
        # h 形状为 (B, C)
        h = self.fc(s)
        
        # 获取 gamma 和 beta，通过torch.chunk分为两个
        # gamma和beta的形状为 (B, C)
        gamma, beta = torch.chunk(h, chunks=2, dim=1)
        # 把形状变为 (B, C, 1)
...
class SwinTransformerBlock(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module, optional): Normalization layer.  Default: nn.InstanceNorm1d
    """
