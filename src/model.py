...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads=4, window_size=8,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 norm_layer=nn.InstanceNorm1d(dim, affine=True))
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, style_dim=64, num_heads=4, window_size=8,
                 mlp_ratio=2., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0.):
        super().__init__()
        self.dim = dim
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 drop=drop, attn_drop=attn_drop,
                                 drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                                 norm_layer=AdaIN(style_dim=style_dim, dim=dim))
            for i in range(depth)])
...
class EncodeBLK(nn.Module):
    def __init__(self, in_dim, out_dim, input_resolution, scale=None, num_heads=4, window_size=8, mlp_ratio=2, depth=2):
        super().__init__()
        self.scale = scale
        self.res = input_resolution
        self.patch_embed = PatchEmbed(img_size=input_resolution, patch_size=1, in_chans=in_dim, embed_dim=in_dim)
        self.transformer = TransformerEncoder(dim=in_dim, input_resolution=[input_resolution, input_resolution], 
                                              num_heads=num_heads, window_size=window_size, mlp_ratio=mlp_ratio, depth=depth)

                                 
        self.patch_unembed = PatchUnEmbed(img_size=input_resolution, patch_size=1, in_chans=in_dim, embed_dim=in_dim)
        if self.scale:
...
class DecodeBLK(nn.Module):
    def __init__(self, in_dim, out_dim, input_resolution, style_dim=64, scale=None, num_heads=4, window_size=8, mlp_ratio=2, depth=2):
        super().__init__()
        self.scale = scale
        if self.scale:
        ...
        self.patch_embed = PatchEmbed(img_size=self.res, patch_size=1, in_chans=out_dim, embed_dim=out_dim)
        self.transformer = TransformerDecoder(dim=out_dim, input_resolution=[self.res, self.res], style_dim=style_dim,
                                              num_heads=num_heads, window_size=window_size, mlp_ratio=mlp_ratio, depth=depth)
        self.patch_unembed = PatchUnEmbed(img_size=self.res, patch_size=1, in_chans=out_dim, embed_dim=out_dim)
...
class Generator(nn.Module):
    def __init__(self, img_size=256, input_channel=3, dim=64, max_dim=512,
                mlp_ratio=2, num_heads=4, patch_size=1, window_size=8, depth=2,
...
class Discriminator(nn.Module):
    def __init__(self, img_size=256, input_channel=3, dim=64, max_dim=512, num_domains=2, 
                mlp_ratio=2, num_heads=4, patch_size=1, window_size=8, depth=2, act_layer=nn.GELU()):
        super().__init__()
        self.img_size = img_size
        self.depth = depth
        self.dim = dim
